...
| Codex `/goal`에 넣을 말이 길거나 어수선함 | 짧고 명확한 목표 프롬프트 작성 |
...
Codex App에서 Threadex를 쓰려면 먼저 Threadex Marketplace를 추가하고, 그 안에서 Threadex plugin을 설치합니다.
...
그 다음 `/agent`에서 Threadex subagents가 보이는지 확인하면 됩니다.
...
| 이름 | 언제 쓰나요? |
| --- | --- |
| `clarify` | 요청이 아직 애매할 때 |
| `specify` | 요구사항을 정리하고 싶을 때 |
| `goal-draft` | Codex `/goal`에 넣을 문장이 필요할 때 |
| `verify` | 끝났다는 주장을 확인하고 싶을 때 |
| `review` | 변경사항에 위험한 문제가 있는지 보고 싶을 때 |
| `compound` | 이번 작업에서 배운 점을 남기고 싶을 때 |
